...
| `api/constants/oneflowTemplates.js` | FALLBACK-lista sedan 2026-07-05 - källan är tabellen `oneflow_templates` via `api/_lib/oneflowTemplates.ts` (server, snapshot-mönster) och `src/services/oneflowTemplateService.ts` (frontend). CommonJS, konvertera INTE till ESM. `src/constants/oneflowTemplates.ts` = frontendens fallback |
...
**Ny Oneflow-mall (dynamiskt sedan 2026-07-05):** lägg till mallen på admin-sidan `/admin/installningar/avtalsmallar` (tabellen `oneflow_templates`) - webhook, nattsync, importer och wizard läser alla därifrån via `api/_lib/oneflowTemplates.ts` (snapshot + 60s-cache) resp. `src/services/oneflowTemplateService.ts`. Mallens fält i Oneflow måste ha custom_id:n för att datainläsningen ska fungera. UNDANTAG som fortfarande är kod: `TEMPLATE_FIELD_ORDER` (webhook.ts, indexbaserad mappning för API-skapade avtal utan custom_id - bara huvudmallen) och `getContractTypeName` (webhook.ts). "Ta bort mall" = avaktivera (wizard-döljning); hård radering är UI-spärrad tills inga avtal refererar mallen.
...
**Felsöka "kontrakt kom inte in":** 1) `oneflow_sync_log` (signaturfel? error?), 2) godkänd mall? (draft + okänd mall skippas tyst), 3) template-retryn kan ta 100 s, 4) kör syncen manuellt: `POST /api/cron/sync-oneflow` med `Authorization: Bearer $CRON_SECRET`, 5) sync-oneflow loggar till `cron_runs` sedan 2026-07-05 (withCronLog) - kolla status/summary där.
...
- **Nattsyncens trashing-mekanik:** allt i DB som inte finns i Oneflow-svaret filtrerat på godkända mallar sätts till `trashed`; `TERMINAL_STATUSES` skyddar bara `declined/ended/trashed` - INTE `signed/active`. Sedan 2026-07-05 läses mallistan STRIKT från `oneflow_templates` (`fetchTemplatesStrict`, ingen fallback): läsfel/tom tabell = hela syncen avbryts som `failed` i cron_runs utan att något trashas, och nya mallar via admin-sidan känns igen direkt. Kvarvarande risk: hård DELETE av en mallrad med kvarvarande avtal (UI spärrar, men rå SQL gör det inte) = avtalen trashas natten efter.
...
2. ~~**Oneflow-mutationsendpoints saknar auth**~~ DELVIS KLART 2026-07-05 (`b8b4759b`): `requireAuth(['admin','koordinator','säljare'])` på create-contract/delete-offer/extend-signing-period/comments, `['admin','koordinator']` på import-contracts/webhook-config; comments-avsändaren härleds från sessionen (x-sender-email borttagen), create-contracts creatorEmail faller tillbaka på auth.email; alla anropare skickar Bearer. KVAR: fil-endpoints (`contract-files*`, `download-file*`, `view-file*` - anropas av `useContracts.ts`) är fortfarande oautentiserade. Rör INTE `webhook.ts` - den autentiserar med signatur, inte JWT.
...
4. ~~**Konsolidera mall-ID:na**~~ KLART 2026-07-05 (dynamiska mallar, gick längre än målbilden): tabellen `oneflow_templates` + admin-sida är källan, alla 7 kopior ersatta av `api/_lib/oneflowTemplates.ts` (snapshot, fallback till legacy-listan vid DB-fel - utom sync-oneflow som är STRIKT och avbryter) och `src/services/oneflowTemplateService.ts`. Syncen wrappad i withCronLog.
