...
| `api/fortnox/proxy.ts` | Generisk proxy `?path=...` mot Fortnox-API:t. Saknar idag auth (känt hål, se Riktning) |
...
- **`CRON_SECRET` failar ÖPPET:** sync-oneflow.ts:280 släpper igenom ALLA anrop om env-varn saknas. Se till att den är satt i Vercel.
...
1. **`api/fortnox/proxy.ts` saknar auth** - öppen läs/skriv-proxy mot bokföringen; vem som helst kan `POST ?path=invoices`. Målbild: `requireAuth(req, res, ['admin','koordinator'])` överst + `Authorization: Bearer`-header i `fortnoxRequest` (`fortnoxService.ts:16`, sessionen finns via supabase-klienten, mönstret finns i `InvoiceDetailModal`) + path-allowlist. Grep:a `fortnox/proxy` i `src/` före merge.
