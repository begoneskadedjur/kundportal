...
4. **Tidszonsfixar i skrivvägarna** (schemalaggning-tidszoner). Belägg: fem levande felskrivningar med `toSwedishISOString` (bl.a. `EditCaseModal.tsx:1023`, kan tippa ärenden i fel provisionsmånad) och cronens UTC-bygge (`extend-recurring-schedules.ts:212`) driftar besök 1-2 timmar. Första steg: byt de fem skrivvägarna till `toLocalISOStringWithOffset`, märk funktionen `@deprecated` för skrivning; cronens bygge rättas med `fromZonedTime('Europe/Stockholm')`.
