...
1. **Auth på service-role-endpoints (säkerhetshål, gör FÖRST)**. Dagens läge: `api/update-multisite-user-email.ts` och `api/create-multisite-users.ts` saknar auth helt (CORS `*`); vem som helst kan byta e-post på VALFRI auth-user = kontokapning. Målbild: bryt ut Bearer-valideringen från `api/multisite-users.ts:33-65` till delad helper och kräv den i alla tre endpoints (även `send-multisite-invitation`); verifiera dessutom att mål-userId tillhör anroparens org. Anropare som måste börja skicka token: `UserModal.tsx:222,239`, `ConvertToMultisiteInline.tsx:256` (wizarden skickar redan). Ingår i säkerhetsplanens fas 1. Behåll 500 ms-sleepen.
