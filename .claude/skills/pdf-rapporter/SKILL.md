...
- Sedan 2026-07-05 (`9a8587ae`) har alla fyra rapport-endpoints (`generate-case-report-pdf`, `generate-inspection-report-pdf`, `generate-work-report`, `send-work-report`) `requireAuthenticated` - samma nivå som `generate-pdf.ts`, ingen rollspärr eftersom kundportalens organisation-vyer också genererar ärenderapporter. Nya PDF-endpoints ska ha `requireAuthenticated` från start, och deras anropare måste skicka Bearer via `getAuthHeaders()`.
...
2. ~~**Lägg till auktorisering**~~ KLART 2026-07-05 (`9a8587ae`): `requireAuthenticated` på alla fyra rapport-endpoints (även generate-work-report + send-work-report), nio frontend-anrop skickar Bearer. Verifierat 4x401 utan token i produktion.
