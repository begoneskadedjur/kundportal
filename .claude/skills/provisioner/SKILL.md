...
- `upsertPostsForCase` raderar poster INNAN valideringen körs (`provisionService.ts:174-197`): sänks priset under 4000 på ett avslutat ärende och sparas raderas posterna, felet blir bara en toast, provisionen är borta. Se Riktning 1.
...
1. **Fixa tyst dataförlust i `upsertPostsForCase` (hög prioritet, låg risk).** Dagens läge: delete körs före valideringen och `EditCaseModal` upsertar vid varje sparning. Målbild: bryt ut tröskel- + andelsvalidering och kör den FÖRE delete, lägg unikt index på `(case_id, technician_id)` via migration (saknas idag, delete+insert i två steg kan ge dubbletter vid parallella sparningar). Långsiktigt: delete+insert som Postgres-RPC för äkta atomicitet.
