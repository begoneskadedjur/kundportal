...
- ~~**Fel i adhoc-fakturaskapandet sväljs**~~ FIXAT 2026-07-05 (`ff11a923`, `74ec1f03`): cbi-raderna skapas med EN array-insert (allt eller inget), `generateAdhocInvoiceForCase` kastar vid DB-fel (null = enbart "inget att fakturera"), `createAdHocItemsFromCase` returnerar `invoiceId/invoiceError` och båda anroparna visar toast-varning. `UnbilledAdhocBanner` i Merförsäljning-fliken listar rader utan `invoice_id` med återkörningsknapp.
...
1. ~~**Adhoc-felsväljningen**~~ KLART 2026-07-05 (`ff11a923` + `74ec1f03`, se Fallgropar). Kvarstående option: nattlig backfill-cron - kräver att generatorn bryts ut till beroendefri modul (src/services kraschar i Node-runtime pga import.meta.env), gör ihop med punkt 2.
