...
| 5 Städning | **Byggt 2026-09-02** (ej browser-testat): faktureringsmodalen med avtal i scope visar start, slut, fakturamånad, frekvens och årspremie som läsning med länken "Öppna avtalskartan" och skriver bara faktureringsläget (på/paus) till avtalet; frekvens och fast avtalsvärde är låsta där; den lokala fakturaschema-vyn och live-diffen göms (planen bor i § 7); datumpropageringen till enheter är borttagen (enheter ärver via huvudkontoret); migration `20260902_avtalskarta_motor_fas5`: `contracts.billing_paused_until` (§ 7 visar "pausad till") och cron `reactivate-paused-billing` återaktiverar både kundrader och avtal; triggern `compute_total_contract_value` på customers nollar `total_contract_value` när kunden har fler än ett levande avtal; `apply_contract_addition` v2 gjordes redan i fas 2. Synth-kunderna lämnas orörda (beslut 10) | S | 2 | ja | efter juli 2027 |
