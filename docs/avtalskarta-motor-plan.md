...
|---|---|---|---|---|---|
| 0 Grund | RPC-gren i resolvern; `getServicePricesForCase` tar `contractId` (ärendets avtal via `cases.contract_id`); `contractBillingService` använder resolvern. **Byggt 2026-09-02.** RPC-migrationen `20260902_contract_candidates_rpc.sql` ligger i repot men kunde inte appliceras automatiskt (SECURITY DEFINER blockerades av verktygets skydd); resolvern faller tillbaka på direktläsning tills den körs för hand | S | inget | RPC | nu |
...
| 2 Pengar och referenser | M3, M4, M7-backfyll; delad planerare med trappa; `uncovered` i stället för betald historik; § 4-rader på riktiga avtal; gemet + samlad faktura (M2); § 8 Referenser (M1, N5) + Fortnox-referensregler; cron på avtalsnivå; historikflik Fakturor; importmodal med avtalsval; WBAB F-642-datafix | L | 1 | ja | april 2027 (FEV/WBAB andra årspremie i juli) |
| 3 Besök, utrustning, löptid | § 3 per enhet (M7 contract_sites); § 6 Utrustning (M5, N4) med per-år-rader och per-runda-koppling; § 9 option + `contract-renewal-watch` + schemahorisont | M | 1, § 6 kräver 2 | ja | juli 2027 |
| 4 Ingår i avtalet + resten av drag och släpp | `covered_by_contract` i ärendeflödet (§ 4 styr ärendet); Katalog-facket med N2, N3; N6, N7; Ångra på alla toaster | M | 0, 1, 3 | ja | hösten 2027 |
