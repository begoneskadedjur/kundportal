// src/services/statisticsService.ts - SLUTGILTIG VERSION MED NYTT EXPORT-MÖNSTER

import { supabase } from '../lib/supabase'
import { Database } from '../types/database'

// Typer och Interfaces är oförändrade...
...
// FIX: Vi exporterar klassen direkt, inte en instans av den.
export class StatisticsService {
    ...
    async getTechnicianStats(periodInDays: number = 30): Promise<TechnicianStats> {
        try {
            ...
            return { activeTechnicians: allTechnicians.length, activeCases: activeCasesList.length, capacityUtilization, averageResolutionTime: this.calculateAverageResolutionTime(allCases), overdueCases: this.calculateOverdueCases(allCases), technicianWorkload: this.calculateTechnicianWorkloadImproved(allTechnicians, allCases) }
        } catch (error) {
    ...
    // =================================================================
    // METODER FRÅN economicStatisticsService NU INNE I DENNA KLASS
    // =================================================================
    ...
    async getDashboardStats() {
        try {
            ...
            const technicians = await this.getTechnicianStats(30);
            const [arrByBusinessType, growthAnalysis, upsellOpportunities, arrProjections] = await Promise.all([ this.getARRByBusinessType(allCustomers, allCases), this.getMonthlyGrowthAnalysis(allCustomers), this.getUpsellOpportunities(allCustomers, allCases, 5), this.getARRProjections(allCustomers) ])
    ...
    async getPerformanceStatsForMonth(selectedDate: Date): Promise<PerformanceStats> {
      ...
      const [customersRes, casesRes] = await Promise.all([ supabase.from('customers').select('id, annual_premium, assigned_account_manager').eq('is_active', true), supabase.from('cases').select('price, assigned_technician_id, assigned_technician_name, pest_type, completed_date, customer_id').not('price', 'is', null).gt('price', 0).gte('completed_date', firstDayOfMonth.toISOString()).lte('completed_date', lastDayOfMonth.toISOString()) ]);
      if (customersRes.error) throw customersRes.error;
      if (casesRes.error) throw casesRes.error;
      ...
      const getTechnicianInfo = (c: CaseData): { key: string; name: string } | null => { if (c.assigned_technician_id) return { key: c.assigned_technician_id, name: c.assigned_technician_name || 'Okänt Namn' }; if (c.assigned_technician_name) return { key: c.assigned_technician_name, name: c.assigned_technician_name }; return null; }
      monthCases.forEach(c => { const techInfo = getTechnicianInfo(c); if (!techInfo) return; let current = techMap.get(techInfo.key); if (!current) { current = { displayName: techInfo.name, contractRevenue: 0, caseRevenue: 0, contractCount: 0, caseCount: 0 }; } current.caseRevenue += c.price || 0; current.caseCount++; current.displayName = techInfo.name; techMap.set(techInfo.key, current); });
      activeCustomers.forEach(customer => { const key = customer.assigned_account_manager?.toLowerCase().trim(); if (!key) return; const matchingTechKey = Array.from(techMap.keys()).find(k => techMap.get(k)?.displayName.toLowerCase().trim() === key); if (!matchingTechKey) return; const current = techMap.get(matchingTechKey)!; current.contractRevenue += (customer.annual_premium || 0) / 12; current.contractCount++; techMap.set(matchingTechKey, current); });
      const byTechnician: TechnicianPerformance[] = Array.from(techMap.values()).map(d => ({ name: d.displayName, contractRevenue: d.contractRevenue, caseRevenue: d.caseRevenue, totalRevenue: d.contractRevenue + d.caseRevenue, contractCount: d.contractCount, caseCount: d.caseCount })).sort((a, b) => b.totalRevenue - a.totalRevenue);
      const pestMap = new Map<string, { revenue: number, caseCount: number }>();
      monthCases.forEach(c => { const pestType = c.pest_type || 'Okänt'; const current = pestMap.get(pestType) || { revenue: 0, caseCount: 0 }; current.revenue += c.price || 0; current.caseCount++; pestMap.set(pestType, current); });
    ...
    async getYearlySpend(year: number): Promise<MonthlySpend[]> {
        try { const { data, error } = await supabase.from('monthly_marketing_spend').select('*').gte('month', `${year}-01-01`).lte('month', `${year}-12-31`).order('month', { ascending: true }); if (error) throw error; return data || []; } catch (error) { console.error('Error fetching yearly spend:', error); return []; }
    }
    async getUpsellOpportunities(allCustomers: CustomerData[], allCases: CaseData[], limit: number = 5): Promise<UpsellOpportunity[]> {
        ...
        const opportunities = activeCustomers.map(customer => { const caseRevenue = caseRevenueMap.get(customer.id) || 0; if (caseRevenue === 0 || !customer.annual_premium || !customer.company_name) return null; return { customerId: customer.id, companyName: customer.company_name, annualPremium: customer.annual_premium, caseRevenueLast6Months: caseRevenue, caseToArrRatio: caseRevenue / customer.annual_premium } }).filter((opp): opp is UpsellOpportunity => opp !== null);
