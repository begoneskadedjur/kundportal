// src/components/organisation/OrganisationServiceActivityTimeline.tsx - Service Activity Timeline för organisationer
...
const OrganisationServiceActivityTimeline: React.FC<OrganisationServiceActivityTimelineProps> = ({ 
  ...
  useEffect(() => {
    ...
    // Set up real-time subscription baserat på om vi har customer_id eller organization
    let subscription: any
    
    if (customerId) {
      // Enkel kund-prenumeration
      ...
      // För organisationer behöver vi inte skapa prenumerationer eftersom vi inte har 
      // cases direkt kopplade till organisationer i nuvarande struktur
      // Vi hämtar bara data en gång
    }
  ...
  const fetchCases = async () => {
    try {
      ...
      if (customerId) {
        ...
        // Om vi har site IDs, hämta cases för dessa siter
        // Först måste vi hämta customer_ids för dessa sites
        ...
        if (sites && sites.length > 0) {
          const customerIds = sites.map(s => s.id)
          query = query.in('customer_id', customerIds)
        } else {
          // Inga siter hittades, sätt tom lista
          setCases([])
          setLoading(false)
          return
        }
        ...
        if (orgSites && orgSites.length > 0) {
          const customerIds = orgSites.map(s => s.id)
          query = query.in('customer_id', customerIds)
        } else {
          // Inga siter för organisationen
          setCases([])
          setLoading(false)
          return
        }
      } else {
        // Ingen identifierare tillgänglig
        console.warn('No identifier available for fetching cases')
        setCases([])
        setLoading(false)
        return
      }
      ...
      // Visa inte toast för 400-fel, bara logga
      if (error.code !== '400') {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">
                {organizationId || organization ? 'Organisationens serviceärenden' : 'Era serviceärenden och förfrågningar'}
              </p>
            </div>
          </div>
