...
interface TechnicianMobileNavProps {
  currentPath: string
  onSignOut: () => void
}
  ...
  return (
    <>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 h-16 bg-slate-900/95 backdrop-blur-xl border-t border-slate-700/50 z-40 px-2 pb-safe flex items-center justify-around">
        {mobileBottomItems.map(item => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`flex flex-col items-center justify-center gap-0.5 px-2 py-2 min-h-[48px] rounded-lg transition-all ${isActive ? 'text-[#20c58f]' : 'text-slate-500'}`}
            >
        ...
        <button onClick={() => setMobileMenuOpen(true)} className="flex flex-col items-center justify-center gap-0.5 px-2 py-2 min-h-[48px] rounded-lg text-slate-500">
