// src/components/admin/settings/PriceListColumnSelector.tsx — Kolumnväljare för prislistetabellen

import { useState, useRef, useEffect } from 'react'
import { Settings, ChevronDown } from 'lucide-react'

export interface PriceListTableColumn {
  id: string
  label: string
...
export const PRICELIST_COLUMNS: PriceListTableColumn[] = [
  { id: 'expand',      label: 'Expandera',   defaultVisible: true,  required: true },
  { id: 'name',        label: 'Namn',        defaultVisible: true,  required: true },
  { id: 'description', label: 'Beskrivning', defaultVisible: true,  required: false },
  { id: 'articles',    label: 'Artiklar',    defaultVisible: true,  required: false },
  { id: 'validity',    label: 'Giltighet',   defaultVisible: false, required: false },
  { id: 'status',      label: 'Status',      defaultVisible: true,  required: false },
  { id: 'actions',     label: 'Åtgärder',    defaultVisible: true,  required: true },
]

const STORAGE_KEY = 'begone_pricelist_columns'

export function usePriceListColumnVisibility() {
  const [visibleColumns, setVisibleColumns] = useState<Set<string>>(() => {
    try {
      const stored = localStorage.getItem(STORAGE_KEY)
      if (stored) return new Set(JSON.parse(stored))
    } catch {}
    return new Set(PRICELIST_COLUMNS.filter(c => c.defaultVisible).map(c => c.id))
  })

  const toggleColumn = (columnId: string) => {
    const col = PRICELIST_COLUMNS.find(c => c.id === columnId)
  ...
  const resetToDefaults = () => {
    const defaults = new Set(PRICELIST_COLUMNS.filter(c => c.defaultVisible).map(c => c.id))
...
interface PriceListColumnSelectorProps {
  visibleColumns: Set<string>
  onToggle: (columnId: string) => void
  onReset: () => void
}

export default function PriceListColumnSelector({ visibleColumns, onToggle, onReset }: PriceListColumnSelectorProps) {
  ...
  const hiddenCount = PRICELIST_COLUMNS.filter(c => !c.required && !visibleColumns.has(c.id)).length
