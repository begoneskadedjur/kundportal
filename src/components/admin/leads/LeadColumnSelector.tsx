// src/components/admin/leads/LeadColumnSelector.tsx — Kolumnväljare för leads-tabellen
...
export const LEAD_COLUMNS: LeadTableColumn[] = [
  { id: 'lead',         label: 'Lead & Kontakt',       defaultVisible: true,  required: true },
  { id: 'statusScore',  label: 'Status & Score',       defaultVisible: true,  required: false },
  { id: 'assigned',     label: 'Tilldelad',            defaultVisible: true,  required: false },
  { id: 'value',        label: 'Värde',                defaultVisible: true,  required: false },
  { id: 'nextStep',     label: 'Nästa steg',           defaultVisible: true,  required: false },
  { id: 'activity',     label: 'Aktivitet',            defaultVisible: true,  required: false },
  { id: 'dealVelocity', label: 'Deal Velocity',        defaultVisible: false, required: false },
  { id: 'closingDate',  label: 'Uppskattad deadline',  defaultVisible: false, required: false },
  { id: 'lastUpdated',  label: 'Senast uppdaterad',    defaultVisible: false, required: false },
  { id: 'priority',     label: 'Prioritet (kolumn)',   defaultVisible: false, required: false },
  { id: 'actions',      label: 'Åtgärder',            defaultVisible: true,  required: true },
]

const STORAGE_KEY = 'begone_lead_columns'

export function useLeadColumnVisibility() {
  const [visibleColumns, setVisibleColumns] = useState<Set<string>>(() => {
    try {
      const stored = localStorage.getItem(STORAGE_KEY)
      if (stored) return new Set(JSON.parse(stored))
    } catch {}
    return new Set(LEAD_COLUMNS.filter(c => c.defaultVisible).map(c => c.id))
  })
  ...
  const resetToDefaults = () => {
    const defaults = new Set(LEAD_COLUMNS.filter(c => c.defaultVisible).map(c => c.id))
...
export default function LeadColumnSelector({ visibleColumns, onToggle, onReset }: LeadColumnSelectorProps) {
  ...
  const hiddenCount = LEAD_COLUMNS.filter(c => !c.required && !visibleColumns.has(c.id)).length
