...
// Fördefinierade färger för regionmarkörer på kartan
const REGION_COLORS = [
...
interface PendingRegion {
  tempId: string
  site_name: string   // t.ex. "Region Nord"
  region: string      // kort kod t.ex. "Nord"
  color: string
}
...
export default function ConvertToRegionalCustomerModal({
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const sitesToInsert = regions.map(r => ({
        ...
        region: r.region,
        // Ärv kontaktuppgifter från huvudkunden
  ...
  const steps = [
    ...
    { key: 'confirm' as Step, label: '2. Bekräfta' },
  ]
  ...
  const footer = (
    <div className="flex items-center justify-between px-4 py-2.5">
      <Button
        variant="ghost"
        onClick={() => step === 'regions' ? handleClose() : setStep('regions')}
        disabled={loading}
        ...
        {step === 'regions' && (
          <>
            ...
            <div className="p-3 bg-slate-800/30 border border-slate-700 rounded-xl space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1.5">Markörfärg på karta</label>
        ...
        {step === 'confirm' && (
          <div className="space-y-4">
            ...
            <div className="p-3 bg-[#20c58f]/10 border border-[#20c58f]/30 rounded-xl">
              <div className="flex items-start gap-2 text-xs text-slate-300 space-y-1">
