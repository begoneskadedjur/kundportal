...
export default function CoordinatorDashboardCard({ 
  ...
  return (
    ...
    >
      <div className="h-full bg-slate-900 p-6 rounded-xl border border-slate-800 hover:border-[#20c58f]/50 hover:bg-slate-800/40 transition-all duration-300 transform hover:-translate-y-1 flex flex-col">
        <div className="flex justify-between items-start">
          <div className="p-3 bg-slate-800/80 border border-slate-700 rounded-lg">
            <Icon className="w-7 h-7 text-[#20c58f]" />
          </div>
          {tag && (
            <span className="bg-[#20c58f]/10 text-[#20c58f] text-xs font-bold px-3 py-1 rounded-full">
              {tag}
            </span>
          )}
        </div>
