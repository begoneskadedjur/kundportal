...
import { Building, User, Zap, MapPin, CheckCircle, PlusCircle, ChevronLeft, FileText } from 'lucide-react';
...
interface Suggestion {
  technician_id: string; technician_name: string; date: string; 
  suggested_time: string; travel_time_minutes: number;
  based_on_case: { id: string; title: string; };
}
...
export default function CreateCaseModal({ isOpen, onClose, onSuccess, technicians }: CreateCaseModalProps) {
  ...
  const handleReset = useCallback(() => {
    ...
    setLoading(false); setSubmitted(false); setSuggestionLoading(false);
  }, []);
  ...
  if (submitted) {
    return (
      <Modal isOpen={isOpen} onClose={() => {}} title="Skapat!" size="md"><div className="p-8 text-center"><CheckCircle className="w-16 h-16 text-green-500 mx-auto mb-4" /><h3 className="text-xl font-semibold text-white">Ärendet har skapats</h3></div></Modal>
    );
  }

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={step === 'selectType' ? 'Välj kundtyp' : `Nytt ärende: ${caseType === 'private' ? 'Privatperson' : 'Företag'}`} size={step === 'form' ? "3xl" : "md"} preventClose={loading}>
      <div className="p-6 max-h-[85vh] overflow-y-auto">
        ...
        {step === 'form' && (
          <form id="create-case-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* --- STEG 1: KÄRNINFORMATION FÖR BOKNINGSASSISTENT --- */}
              ...
              {suggestions.length > 0 && (
                <div className="pt-4 border-t border-slate-700 space-y-2">
                  ...
                  {suggestions.map((sugg) => (
                    <div key={sugg.based_on_case.id} className="p-3 rounded-md bg-slate-700/50 hover:bg-slate-700 cursor-pointer transition-colors" onClick={() => applySuggestion(sugg)}>
                      ...
                      <div className="text-sm text-slate-400">{new Date(sugg.date).toLocaleDateString('sv-SE', { weekday: 'long', day: 'numeric', month: 'long' })} {sugg.suggested_time}</div>
                    </div>
                  ))}
                </div>
              )}
