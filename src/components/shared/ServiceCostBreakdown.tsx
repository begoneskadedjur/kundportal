...
//
// Opt-in-props (bakåtkompatibla defaults, ContractInvoiceModal påverkas inte):
// - defaultCollapsed: rendera som hopfällbar "Intern kalkyl"-rad, initialt stängd
// - neutralMargin: raderna är pro rata-avtalstillägg — marginal ej tillämplig,
//   visa neutral text istället för röd procent
...
interface ServiceCostBreakdownProps {
  ...
  /** Rendera som hopfällbar "Intern kalkyl"-rad, initialt stängd (default false) */
  defaultCollapsed?: boolean
  /** Pro rata-läge: marginal ej tillämplig, neutral text istället för röd procent (default false) */
...
export default function ServiceCostBreakdown({
          ...
          <div key={serviceRow.id}>
            ...
            >
              ...
              <div className="flex items-center gap-4 text-xs">
                ...
                {neutralMargin ? (
                  <span className="text-slate-400 min-w-[60px] text-right">Pro rata</span>
                ) : (
  ...
  // Hopfällbart läge (fakturamodalen): kompakt rubrikrad som togglar innehållet
  if (defaultCollapsed) {
    return (
      <div className="bg-slate-800/50 rounded-lg overflow-hidden">
        <button
          ...
          className={`w-full px-3 py-2 flex items-center justify-between hover:bg-slate-700/30 transition-colors text-left ${collapsed ? '' : 'border-b border-slate-700'}`}
        >
