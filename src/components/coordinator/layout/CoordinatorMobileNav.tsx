...
interface CoordinatorMobileNavProps {
  currentPath: string
  onSignOut: () => void
}
  ...
  return (
    <>
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 h-14 bg-slate-900/95 backdrop-blur-xl border-b border-slate-700/50 z-40 px-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <ThemeToggle />
          <Link to="/koordinator/tickets" className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 relative" aria-label="Notifieringar">
      ...
      >
        <nav className="p-3 space-y-1">
          ...
          <ProfileLink basePath="/koordinator" variant="mobile" onNavigate={() => setMobileMenuOpen(false)} />
          <button
            onClick={onSignOut}
