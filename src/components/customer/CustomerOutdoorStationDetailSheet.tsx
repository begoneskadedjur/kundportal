...
export function CustomerOutdoorStationDetailSheet({
  station,
  inspections = [],
  isOpen,
  onClose
  ...
  const getTypeConfig = () => {
    // Försök hämta från station_type_data (dynamisk från databas)
    ...
    return {
      color: '#6b7280',
      label: station.equipment_type || 'Station'
    }
  }
  ...
  const TypeIcon = TYPE_ICONS[station.equipment_type] || Box
              ...
              <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
                ...
                {latestInspection && (
                  <div className="bg-slate-900/70 rounded-xl p-4 border border-slate-700/50">
                    ...
                    <OutdoorInspectionCard inspection={latestInspection} isLatest onPhotoClick={setLightboxPhoto} />
                  </div>
                )}
                ...
                <div className="space-y-3">
                  ...
                  {/* Position - klickbar för att öppna i karta */}
                  <button
                    onClick={() => openInMapsApp(station.latitude, station.longitude)}
                    className="w-full flex items-start gap-3 p-3 bg-slate-900/50 rounded-lg hover:bg-slate-900/70 transition-colors group"
                  >
                    ...
                    <ExternalLink className="w-4 h-4 text-slate-500 group-hover:text-emerald-400 transition-colors mt-0.5" />
                  </button>
...
// Inspektionskort-komponent för utomhusstationer
function OutdoorInspectionCard({
  inspection,
  isLatest = false,
  onPhotoClick
}: {
  inspection: OutdoorInspectionWithRelations
  isLatest?: boolean
...
export default CustomerOutdoorStationDetailSheet
