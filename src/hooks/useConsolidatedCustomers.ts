  ...
  const consolidateCustomers = (
    ...
    consolidatedMap.forEach((org, orgId) => {
      if (org.organizationType === 'multisite') {
        ...
        // Priset läses från huvudkontoret i första hand, annars summera sites som fallback
        const hq = org.headquarterCustomer
        org.totalAnnualValue = (hq?.annual_value ? Number(hq.annual_value) : 0)
          || sites.reduce((sum, site) => sum + (site.annual_value || 0), 0)
        org.totalMonthlyValue = (hq?.monthly_value ? Number(hq.monthly_value) : 0)
          || sites.reduce((sum, site) => sum + (site.monthly_value || 0), 0)
