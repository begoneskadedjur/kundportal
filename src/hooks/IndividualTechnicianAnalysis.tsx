// 📁 src/components/admin/technicians/IndividualTechnicianAnalysis.tsx - ANVÄNDER BEFINTLIG HOOKS ARKITEKTUR
...
export default IndividualTechnicianAnalysis// 📁 src/components/admin/technicians/IndividualTechnicianAnalysis.tsx - INDIVIDUELL TEKNIKER ANALYS
import React, { useState } from 'react'
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, BarChart, Bar, PieChart, Pie, Cell, Legend } from 'recharts'
import { User, TrendingUp, Award, Target, Bug, Calendar, DollarSign, BarChart3 } from 'lucide-react'
import Card from '../../ui/Card'
...
const IndividualTechnicianAnalysis: React.FC = () => {
  ...
  const { data: individualData, loading: loadingIndividual, error } = useIndividualTechnician(selectedTechnicianId)
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-400">Högsta värde ärenden</p>
              <p className="text-slate-400">Utvecklingsmöjligheter</p>
              <p className="text-slate-400">Personliga insights</p>
            </div>
          </div>
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-white">Individuell Tekniker Analys</h2>
              <p className="text-sm text-slate-400">Laddar detaljerad analys...</p>
            </div>
          </div>
          <Button
            variant="secondary"
  ...
  const { technician, monthly_trends, pest_specializations, comparison_metrics, top_cases } = individualData

  // Förbered chart data
  const monthlyChartData = monthly_trends
    .sort((a, b) => a.month.localeCompare(b.month))
    .map(m => ({
      month: m.month.slice(5), // MM format
      revenue: m.total_revenue,
  ...
  // Specialisering pie data
  const pestPieData = pest_specializations
    .reduce((acc, spec) => {
      const existing = acc.find(p => p.name === spec.pest_type)
      if (existing) {
        existing.value += spec.total_revenue
  ...
  const comparisonData = [
    {
      metric: 'Intäkt vs Team',
      value: comparison_metrics.revenue_vs_team_avg,
      color: comparison_metrics.revenue_vs_team_avg >= 0 ? '#22c55e' : '#ef4444'
    },
    {
      metric: 'Ärenden vs Team',
      value: comparison_metrics.cases_vs_team_avg,
      color: comparison_metrics.cases_vs_team_avg >= 0 ? '#22c55e' : '#ef4444'
    },
    {
      metric: 'Ärendepris vs Team',
      value: comparison_metrics.avg_case_value_vs_team,
      color: comparison_metrics.avg_case_value_vs_team >= 0 ? '#22c55e' : '#ef4444'
    }
  ]
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          <TrendingUp className="w-5 h-5 text-green-500" />
          Månadsvis Prestanda Utveckling
          <span className="text-sm text-slate-400 font-normal">({monthly_trends.length} månader)</span>
        </h3>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={monthlyChartData}>
                ...
                <YAxis stroke="#9ca3af" fontSize={12} tickFormatter={(value) => formatCurrency(value).replace(' kr', 'k')} />
                <Tooltip content={<CustomTooltip />} />
                <Line type="monotone" dataKey="revenue" stroke="#22c55e" strokeWidth={3} dot={{ fill: '#22c55e', strokeWidth: 2, r: 4 }} />
                <Line type="monotone" dataKey="private" stroke="#8b5cf6" strokeWidth={2} strokeDasharray="5 5" />
                <Line type="monotone" dataKey="business" stroke="#3b82f6" strokeWidth={2} strokeDasharray="5 5" />
                <Line type="monotone" dataKey="contract" stroke="#f59e0b" strokeWidth={2} strokeDasharray="5 5" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          ...
          <div className="mt-6 p-4 bg-slate-800/50 rounded-lg border border-slate-700">
            ...
            {comparison_metrics.revenue_vs_team_avg > 20 && (
              <p className="text-green-400 text-sm mt-1">🏆 Toppresteration - Betydligt över teamgenomsnittet!</p>
            )}
            {comparison_metrics.revenue_vs_team_avg < -20 && (
              <p className="text-yellow-400 text-sm mt-1">💡 Utvecklingspotential - Under teamgenomsnittet</p>
      ...
      <Card className="p-6">
        <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          ...
          Högsta Värde Ärenden
          <span className="text-sm text-slate-400 font-normal">(Topp {Math.min(top_cases.length, 5)})</span>
        </h3>
        
        {top_cases.length > 0 ? (
          <div className="space-y-3">
            {top_cases.slice(0, 5).map((case_item, index) => (
              <div 
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-semibold text-white">{case_item.title}</h4>
                    <div className="flex items-center gap-3 text-sm text-slate-400">
                      <span>{case_item.source}</span>
                      <span>•</span>
                      <span>{new Date(case_item.date).toLocaleDateString('sv-SE')}</span>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-white">{formatCurrency(case_item.revenue)}</p>
                  <p className="text-xs text-slate-400">Case #{case_item.case_id.slice(-6)}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="text-center py-8 text-slate-400">
            ...
            <p>Inga högvärde ärenden registrerade för {technician.name}</p>
          </div>
        )}
      </Card>
      ...
      <Card className="p-6 bg-gradient-to-br from-slate-800/50 to-slate-900/50 border-slate-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
          <div>
            <h4 className="text-slate-300 font-medium mb-2">🎯 Styrkör</h4>
            <p className="text-slate-400">
              {pest_specializations.length > 0 
                ? `Expertis inom ${pest_specializations.sort((a, b) => b.total_revenue - a.total_revenue)[0]?.pest_type || 'okänt område'}`
                : 'Bred kompetens över flera områden'
              }
            </p>
            {technician.avg_case_value > allTechnicians.reduce((sum, t) => sum + t.avg_case_value, 0) / allTechnicians.length && (
              <p className="text-green-400 mt-1">Högt ärendepris indikerar kvalitetsarbete</p>
            )}
          </div>
          ...
          <div>
            <h4 className="text-slate-300 font-medium mb-2">💡 Utvecklingsområden</h4>
            {comparison_metrics.revenue_vs_team_avg < 0 && (
              <p className="text-yellow-400">Intäkt under teamgenomsnittet - fokusera på värdeskapande</p>
            )}
            {comparison_metrics.cases_vs_team_avg < 0 && (
              <p className="text-yellow-400">Färre ärenden än teamet - öka aktivitetsnivån</p>
            )}
            {comparison_metrics.avg_case_value_vs_team < 0 && (
              <p className="text-yellow-400">Lägre ärendepris - utforska premium-tjänster</p>
            )}
            {comparison_metrics.revenue_vs_team_avg >= 0 && comparison_metrics.cases_vs_team_avg >= 0 && comparison_metrics.avg_case_value_vs_team >= 0 && (
              <p className="text-green-400">Stark prestanda över alla områden!</p>
            )}
          </div>
          ...
          <div>
            ...
            {pest_specializations.length < 3 && (
              <p className="text-blue-400">Utöka specialiseringar inom nya skadedjurstyper</p>
            )}
            {monthly_trends.length >= 3 && (
              <p className="text-purple-400">Stabil utveckling - fokusera på effektivisering</p>
            )}
          </div>
        </div>
