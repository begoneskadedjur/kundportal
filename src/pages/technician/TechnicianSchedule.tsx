...
// ⭐ VERSION 10.0 - INTEGRERAD RAPPORT & ANALYS-FUNKTION ⭐
// Denna version integrerar den nya ReportModal-komponenten för djupare analys.
// 1. Ny knapp: En "Rapport & Analys"-knapp har lagts till i gränssnittet.
// 2. Modal-integration: Logik för att öppna och stänga den nya modalen är implementerad.
// 3. Interaktivitet: Det är nu möjligt att klicka på ärenden i rapporten för att öppna dem.
...
import { ArrowLeft, Calendar, Phone, MapPin, ChevronLeft, ChevronRight, User, Users, Clock, AlertCircle, Navigation, Search, Filter, X, BarChart } from 'lucide-react' // NYTT: BarChart ikon
import Button from '../../components/ui/Button'
import Card from '../../components/ui/Card'
import EditCaseModal from '../../components/admin/technicians/EditCaseModal'
import LoadingSpinner from '../../components/shared/LoadingSpinner'
import ReportModal from '../../components/admin/technicians/ReportModal' // NYTT: Importerar den nya modalen
...
const AgendaCaseItem = ({ caseData, onOpen }: { caseData: ScheduledCase, onOpen: (c: ScheduledCase) => void }) => {
    ...
    return (
        <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, transition: { duration: 0.2 } }} className={`bg-slate-900 border ${colors.border} rounded-xl overflow-hidden shadow-lg hover:shadow-blue-500/10 transition-shadow`}>
            <div className={`px-4 py-3 flex items-center justify-between border-b ${colors.border} ${colors.bg}`}><div className="flex items-center gap-3"><span className="font-mono text-base font-bold text-white tracking-wider">{timeSpan}</span><span className={`px-2.5 py-1 text-xs font-semibold rounded-full border ${colors.border} bg-slate-950/50 ${colors.text}`}>{status}</span></div>{getCaseTypeIcon(case_type)}</div>
...
export default function TechnicianSchedule() {
  ...
  const casesForSelectedDay = useMemo(() => filteredCases.filter(c => c.start_date.startsWith(selectedDate)).sort((a, b) => new Date(a.start_date).getTime() - new Date(b.start_date).getTime()), [filteredCases, selectedDate]);
  
  const eventsByDay = useMemo(() => filteredCases.reduce((acc, event) => { const day = event.start_date.split('T')[0]; if (!acc[day]) acc[day] = 0; acc[day]++; return acc; }, {} as Record<string, number>), [filteredCases]);
  ...
  // NYTT: Funktion för att hantera klick från rapportmodalen
  const handleOpenCaseFromReport = (caseData: ScheduledCase) => {
    setIsReportModalOpen(false); // Stäng rapportmodalen först
    handleOpenModal(caseData); // Återanvänd befintlig funktion för att öppna redigeringsmodalen
  };
      ...
      <div className="min-h-screen bg-slate-950 text-white flex flex-col">
        ...
        <div className="flex-grow max-w-screen-2xl mx-auto w-full p-2 sm:p-4 flex lg:flex-row flex-col gap-4">
          ...
          <main className="flex-grow w-full lg:w-2/3 xl:w-3/4">
            ...
            {/* ÄNDRING: Lade till Rapport-knappen */}
            <div className="mb-4 flex gap-2">
              ...
              <Button variant="secondary" onClick={() => setIsReportModalOpen(true)} title="Rapport & Analys">
