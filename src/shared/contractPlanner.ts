...
/** § 6-rad med faktureringsläge "per styck och år". Belopp per år. */
...
export function computePlannedPeriods(contract: PlanningContract, opts: PlanOptions = {}): PlannedPeriod[] {
  ...
  const hasEquipment = !!opts.equipment && opts.equipment.length > 0
  ...
  return intervals.map(({ periodStart, periodEnd }, idx) => {
    ...
    const equipmentRows: PlannedEquipmentRow[] = (opts.equipment ?? [])
      .filter((e) => e.quantity > 0 && e.unit_price_annual > 0)
      .map((e) => {
